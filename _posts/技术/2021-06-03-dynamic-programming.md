...
大多数动态规划问题本质都是递归的问题，递归的过程中存在重叠子问题（即存在相同情况重复计算），也存在最优子结构（性质：针对求解的问题是求解一个最优化的问题，要求得整个问题的最优解只需求出所有子问题的最优解，求子问题最优解的过程中还存在重叠子问题的形式）。对于重叠子问题，一可以采用记忆化搜索来解决（自顶向下的解决问题），二可以采用动态规划来解决（自底向上的解决问题，本质上和记忆化搜索是一样的）。  
